...
inline int nextPrimeNumber(int n) {
    ...
    while (!isPrimeNumber(candidate)) {
        if (candidate > std::numeric_limits<int>::max() - 2) throw std::overflow_error("hash capacity is too large");
        candidate += 2;
    }
    return candidate;
}
...
public:
    ...
    explicit HashMap(int initCapacity = 11, double maxLoad = 0.75, Hasher h = Hasher())
        ...
        if (maxLoadFactor <= 0.0 || maxLoadFactor >= 1.0) throw std::invalid_argument("load factor must be between zero and one");
...
protected:
    ...
    void rehash(int newCapacity) {
        ...
        for (int i = 0; i < oldCapacity; ++i) {
            ...
            if (!e.kv.has_value()) continue;
            size_t start = indexOf(e.kv->first);
            for (int offset = 0; offset < capacity; ++offset) {
                size_t index = (start + static_cast<size_t>(offset)) % static_cast<size_t>(capacity);
                if (!table[index].kv.has_value()) {
                    table[index].kv = std::move(e.kv.value());
                    ++length;
                    break;
                }
            }
