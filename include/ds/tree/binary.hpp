...
public:
    ...
    std::expected<void, DataStructureError> betweenRight(Node* parent, const T& value) {
        ...
        Node* newNode = new Node{value, parent, nullptr, parent->right};
    ...
    std::expected<bool, DataStructureError> isBalanced(Node* node = nullptr) const {
        ...
        std::function<int(Node*)> balancedHeight = [&](Node* current) {
            if (current == nullptr) return 0;
            int leftHeight = balancedHeight(current->left);
            if (leftHeight < 0) return -1;
            int rightHeight = balancedHeight(current->right);
            if (rightHeight < 0 || std::abs(leftHeight - rightHeight) > 1) return -1;
            return std::max(leftHeight, rightHeight) + 1;
        };
        return balancedHeight(node) >= 0;
    }
...
public:
    ...
    std::expected<void, DataStructureError> remove(const T& value) {
        DS_TRY(rebalanceStart, BinarySearchTree<T>::remove(value));
